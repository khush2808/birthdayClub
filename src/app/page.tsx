...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage(
          "Registration successful! You will receive birthday wishes from all friends.",
        );
  ...
  return (
    <div
      className={`min-h-screen bg-gradient-to-br ${currentThemeData.bg} flex flex-col items-center justify-center p-4 relative overflow-hidden transition-all duration-700`}
    >
      {/* Background decorative elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div
          className={`absolute -top-40 -right-40 w-80 h-80 ${currentThemeData.elements[0]} rounded-full mix-blend-multiply filter blur-xl opacity-30 animate-pulse transition-all duration-700`}
        ></div>
        <div
          className={`absolute -bottom-40 -left-40 w-80 h-80 ${currentThemeData.elements[1]} rounded-full mix-blend-multiply filter blur-xl opacity-30 animate-pulse delay-1000 transition-all duration-700`}
        ></div>
        <div
          className={`absolute top-40 left-1/2 w-60 h-60 ${currentThemeData.elements[2]} rounded-full mix-blend-multiply filter blur-xl opacity-40 animate-pulse delay-500 transition-all duration-700`}
        ></div>
      </div>
      ...
      <div className="relative backdrop-blur-xl bg-white/25 border border-white/40 rounded-2xl shadow-2xl p-8 w-full max-w-md z-10">
        ...
        {message && (
          <div
            className={`mt-6 p-4 rounded-xl text-sm backdrop-blur-sm border ${
